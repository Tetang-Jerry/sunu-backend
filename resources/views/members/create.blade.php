...
@section('content')
    <div class="space-y-6 max-w-3xl">
        <div>
            <h1 class="font-heading text-2xl text-gray-900">Nouveau membre</h1>
            <p class="text-gray-500">Ajoutez un membre</p>
        </div>

        <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-6">
            <form action="{{ route('members.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @csrf
                <div>
                ...
                <div class="md:col-span-2 flex items-center justify-end gap-2 pt-2">
                    <a href="{{ route('members.index') }}" class="px-4 py-2 border rounded-lg hover:bg-gray-50">Annuler</a>
                    <button type="submit"
                        class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg">Créer</button>
                </div>
            </form>
        </div>
    </div>
