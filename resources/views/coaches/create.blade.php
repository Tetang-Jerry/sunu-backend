  ...
  <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-6">
    <form action="{{ route('coaches.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div class="md:col-span-2">
        ...
        <input type="hidden" name="availability_json" id="availability_json" value='@json(old('availability_json') ? json_decode(old('availability_json'), true) : [])'>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function render() {
      ...
      Object.keys(byDate).sort().forEach(date => {
        ...
        byDate[date].forEach((slot, idx) => {
          const chip = document.createElement('span');
          chip.className = 'inline-flex items-center gap-2 px-2 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs';
          ...
          holder.className = 'inline-flex items-center gap-1';
          holder.appendChild(document.createTextNode(slot+' '));
