...
<div class="space-y-6 max-w-3xl">
  <div class="flex items-center justify-between">
    <div>
      <h1 class="font-heading text-2xl text-gray-900">Séance — {{ optional(optional($session->coach)->user)->first_name }} {{ optional(optional($session->coach)->user)->last_name }}</h1>
      <p class="text-gray-500">Détails de la séance</p>
    </div>
    <div class="flex items-center gap-2">
      <a href="{{ route('sessions.index') }}" class="px-4 py-2 border rounded-lg hover:bg-gray-50">Retour</a>
      <a href="{{ route('sessions.edit', $session) }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg">Modifier</a>
      <form action="{{ route('sessions.destroy', $session) }}" method="POST" class="inline delete-form">
        @csrf
        @method('DELETE')
        <button type="button" class="delete-btn px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg" data-session-label="{{ optional(optional($session->coach)->user)->first_name }} {{ optional(optional($session->coach)->user)->last_name }} — {{ optional($session->date_time)->format('d/m/Y H:i') }}">Supprimer</button>
      </form>
    </div>
  </div>
  ...
  <div class="bg-white rounded-2xl shadow-md border border-gray-200 p-6 space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <div class="text-sm text-gray-500 mb-1">Coach</div>
        <div class="text-gray-900 font-medium">{{ optional(optional($session->coach)->user)->first_name }} {{ optional(optional($session->coach)->user)->last_name }}</div>
        <div class="text-gray-600 text-sm">{{ optional(optional($session->coach)->user)->email }}</div>
      </div>
      <div>
        <div class="text-sm text-gray-500 mb-1">Membre</div>
        <div class="text-gray-900 font-medium">{{ optional($session->user)->first_name }} {{ optional($session->user)->last_name }}</div>
        <div class="text-gray-600 text-sm">{{ optional($session->user)->email }}</div>
      </div>
      <div>
        <div class="text-sm text-gray-500 mb-1">Date & Heure</div>
        <div class="text-gray-900">{{ optional($session->date_time)->format('d/m/Y H:i') }}</div>
      </div>
      <div>
        <div class="text-sm text-gray-500 mb-1">Durée</div>
        <div class="text-gray-900">{{ $session->duration }} min</div>
      </div>
      <div>
      ...
      <div>
        ...
        <div class="text-gray-900">{{ $session->status }}</div>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
      <div class="sm:flex sm:items-start">
        ...
        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
          ...
          <div class="mt-2">
            <p class="text-sm text-gray-500" id="sessionLabel"></p>
          </div>
        </div>
      </div>
    </div>
